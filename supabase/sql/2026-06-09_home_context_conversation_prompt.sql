-- Adds lightweight session conversation guidance to the Admin-managed Home
-- context prompts. Safe to re-run.
  ...
  loop
    ...
    if matching_version_id is not null then
      ...
      values (
        ...
        target_temperature,
        true,
        'Add lightweight session conversation guidance',
        target_content_hash
      );
    end if;
  end loop;
