...
type EmailNotificationStatus = "failed" | "not_queued" | "queued" | "sent";
...
async function updateMessageEmailStatus(
  supabase: unknown,
  messageId: string,
  ...
  const { error } = await (
    ...
    "update_support_ticket_message_email_status",
    {
  ...
  if (error) {
    console.error("Could not update support email status", error);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!accessToken) {
      throw new Error("Please sign in before sending support notifications.");
    }
    ...
    if (!messageId) {
      throw new Error("Support message id is required.");
    }
    ...
    if (profile?.is_admin !== true) {
      throw new Error("Admin access is required to send support notifications.");
    }

    const { data: message, error: messageError } = await supabase
      .from("support_ticket_messages")
      .select("id,ticket_id,author_role,is_internal,email_notification_status")
      .eq("id", messageId)
      .single();
    ...
    if (
      ...
      message.is_internal ||
      message.email_notification_status === "sent"
    ) {
    ...
    if (!recipientProfile?.email) {
      await updateMessageEmailStatus(supabase, message.id, "failed");
    ...
    if (
      ...
      await updateMessageEmailStatus(supabase, message.id, "not_queued");
    ...
    await updateMessageEmailStatus(supabase, message.id, "queued");
    ...
    await updateMessageEmailStatus(supabase, message.id, "sent");
