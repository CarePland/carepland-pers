...
- Profile display labels must not fall back to the raw account email for person-facing Care Circle labels. Prefer explicit display name, then first/last name, then a neutral account label such as `You`; email remains an account identifier, not a default visible person name. Stored display names that match the auth email should hydrate as blank display-name drafts.
...
- The first-run welcome guide's fictional demo/example-data CTA should keep progress visible until sample seeding succeeds, then dismiss the guide and open Appointments with freshly loaded sample appointments. It should not dismiss first and leave users on a blank normal Home/Appointments surface while seeding is pending or failed.
...
- Personal Ask conversations are not unsaved work merely because messages are visible. Dirty-state protection should fire only for typed-but-unsent Ask input or an in-flight Ask response. A completed or read-only Ask exchange must be safe to leave without a browser leave-site warning.
- Manual appointment creation is intentionally low-friction: the durable required fields are Care VIP/person, appointment title, and date/time. Provider, practice, address/location, phone, and reason add context but should remain optional for save/submit unless the product explicitly changes the appointment data contract. The signed-in app should render only one manual Add Appointment form at a time; hidden duplicate submit controls are not acceptable.
...
- Browser-side Supabase auth/data access should reuse the singleton client in `app/lib/platform/browserSupabase.ts`. Server/API routes may still create request-scoped clients with explicit auth/service-role headers. Do not create new browser `createClient(...)` instances at module scope because multiple GoTrue clients in one browser context can produce undefined auth behavior.
