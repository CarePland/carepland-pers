...
- Connect call transcripts are temporary by design. Store the transcript on the call record only until the AI-generated Call Summary is presented and approved; once approved, permanently delete the associated transcript while keeping the approved summary attached to the call. Approval should be requested from both call parties on completion when possible. Approval may come from either party; the Receiver participant may be excluded from approval when the flow explicitly chooses that because cognitive impairment or caregiving context may make the managing party the more appropriate approver. If both parties approve different/refined summaries, preserve both approved summaries rather than forcing one canonical version; that discrepancy may become useful later for preference interpretation, care-context nuance, or possible cognitive/functional signal review. The first implementation currently approves from the Receiver post-call summary popup with `Approve` and `Not Quite`; `Approve` calls the app-owned summary approval endpoint and clears local transcript text, while `Not Quite` is only a placeholder for clarification/refinement right now. Crucial TODOs: define cleanup for unapproved transcripts, define dual-party approval UX, define discrepancy handling, define clarification/refinement workflow, and define the long-term approval surface if it moves to Coordinator, Call History, or another review workflow.
...
- The standalone Connect Receiver Android direction is a thin native appliance shell, not a native rewrite of Connect. Keep the native shell under `android/connect-receiver`; it owns launch/provisioning, permissions, wake/reboot behavior, kiosk/device-owner hooks, WebView/TWA settings, native device/screen detection, and future hardware-specific behavior. The hosted web Receiver remains the product surface and owns UI/layouts, fixed-resolution device profiles, copy, remote config, receiver-user changes, calls, messages, diagnostics, and most feature updates. Provisioning should be web-first: a simple setup URL opens an authenticated Supabase-backed approval page, verifies a short-lived setup code, lets an authorized user approve the install and phase-appropriate hardware profile, then opens the APK with a short-lived native app claim. Human-typed setup URLs should support short word phrases such as `/r/kind-maple-chair` while keeping the underlying token opaque and separate. Provisioning links must never carry permanent account credentials. Server-side receiver binding, revocation, and token rotation remain the target durable model. Default phone/receiver behavior, kiosk, and hardware setting control should be phased after real-device testing. See `docs/CONNECT_RECEIVER_ANDROID_SHELL.md`.
