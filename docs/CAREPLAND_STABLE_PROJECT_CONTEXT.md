...
- Health Focus story details should feel snappy after the first load. Cache loaded topic detail responses in memory for the current browser tab/session with a small cap, prefetch the initially visible topics after summary load, and prefetch hidden topics when Show all expands. Do not store full story details in browser storage.
...
- Connect call transcripts are temporary by design. Store the transcript on the call record only until the AI-generated Call Summary is presented and approved; once approved, permanently delete the associated transcript while keeping the approved summary attached to the call. Approval should be requested from both call parties on completion when possible. Approval may come from either party; the Receiver participant may be excluded from approval when the flow explicitly chooses that because cognitive impairment or caregiving context may make the managing party the more appropriate approver. The Receiver approval flow should include a user-edit stage: the model-generated summary is shown as a draft, the user may modify or add care-relevant text, and approval stores the edited/user-approved summary while preserving the original model-generated summary for later comparison and audit of what the model summarized. If both parties approve different/refined summaries, preserve both approved summaries rather than forcing one canonical version; that discrepancy may become useful later for preference interpretation, care-context nuance, or possible cognitive/functional signal review. The first implementation currently approves from the Receiver post-call summary popup; `Approve` calls the app-owned summary approval endpoint with the edited summary and clears local transcript text, while `Not Quite` only exposes an edit/refinement reminder and does not yet regenerate. Crucial TODOs: define cleanup for unapproved transcripts, define dual-party approval UX, define discrepancy handling, define richer clarification/regeneration workflow, and define the long-term approval surface if it moves to Coordinator, Call History, or another review workflow.
...
- Durable Connect call storage is defined in `supabase/sql/2026-06-25_connect_call_records.sql`: `connect_calls` for call lifecycle and temporary transcript text, `connect_call_signals` for short-lived WebRTC signaling, `connect_call_events` for diagnostics without transcripts/audio, and `connect_call_summaries` for brief care-only summaries/approval. `supabase/sql/2026-06-25_connect_call_summary_prompt.sql` seeds the Admin-managed `connect_call_care_summary` prompt. The app-owned call routes now try Supabase-backed call/signaling storage first and fall back to local JSON storage when the durable tables are unavailable, preserving local development while making deployed remote demos possible after the SQL is applied.
...
- Dedicated Receiver mode owns appliance-style recovery behavior in the native shell: boot/package-replaced auto-launch is enabled only after the local provisioning wizard records `receiver_mode = dedicated`, and the app attempts a short-delay soft reopen if it is pushed to the background. Personal Device mode must remain a normal Android app and should not auto-launch or aggressively return to the foreground.
