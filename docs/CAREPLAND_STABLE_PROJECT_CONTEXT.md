...
- Android 7-era appliance WebViews such as the Grandstream/GXV3370 class and iTab-style Android 7 kiosk displays may not reliably run the modern React/Next Receiver client bundle. The near-term Receiver strategy is one APK with strict hosted renderer separation: modern React work stays under `/connect/receiver`, while old-WebView appliance work stays under `/connect/receiver/legacy` and uses plain server-rendered HTML/CSS plus Android-7-compatible JavaScript. Do not split APKs solely for UI/layout differences, and do not blur legacy/modern implementation directories with scattered conditional UI branches. Legacy is now a real appliance renderer to polish first for current test hardware, not merely a temporary error page or mockup.
