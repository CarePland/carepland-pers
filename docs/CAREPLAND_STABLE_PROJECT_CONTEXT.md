...
- Example-data onboarding is a staged product workflow, not a silent background task. It runs core sample data first, then the meaning layer, then Receiver/Messages examples; the UI should show progress, leave retry available after failure, surface a calm error, and log the failing stage through Admin-diagnosable operational records.
