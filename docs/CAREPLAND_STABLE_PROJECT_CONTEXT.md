...
- Receiver Modern is an experimental hosted React Receiver presentation for adult children, family caregivers, tech-comfortable adults, self-use, browsers, tablets, and larger phones. It is selectable with `receiverLayout=modern` and must remain presentation-only: no API, data model, provisioning, authorization, CCKL, HKL, Intent Router, Decision Trace, or workflow changes. Receiver Classic remains the default and reference implementation for seniors, cognitive accessibility, assisted living, and dedicated appliances. See `docs/RECEIVER_MODERN_EXPERIMENT.md`.
