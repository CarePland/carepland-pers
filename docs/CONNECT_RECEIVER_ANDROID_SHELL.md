...
- Browser-opened Receivers use the same pairing-session model without requiring the Android APK: an unpaired `/connect/receiver` browser session creates a browser-local receiver install id, requests a six-digit pairing code, displays it on the Receiver screen, polls for pairing, redeems the paired claim, and then stores the bound receiver-device identity locally. This is the normal web Receiver path.
- Connect Dashboard's normal user-facing path is Connect -> Receiver -> Pair Receiver. The caregiver enters the six-digit code shown on the web Receiver or APK Receiver, and the pairing route binds that Receiver to the current Main Connect User / Receiver Active Person from the signed-in Connect app context.
- `/connect/receiver/setup` remains the install-from-link/public setup page: its primary flow is Install Receiver -> open Receiver and get code -> enter code to pair, with setup-page QR, dedicated-device QR, local/debug claim creation, and emulator tools under Advanced setup. It should keep working for direct/public setup links, but it is not the primary in-app browser pairing surface.
